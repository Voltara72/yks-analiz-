...
def programi_yukle():
    ...
    return pd.DataFrame({"Gün": gunler, "Sabah Blok": [""]*7, "Öğle Blok": [""]*7, "Akşam Blok": [""]*7})
...
if not aktif_hatirlaticilar.empty:
    for _, row in aktif_hatirlaticilar.iterrows():
        st.toast(f"🔔 HATIRLATICI: Saat {row['Saat']} - {row['Görev / Ders']} zamanı!", icon="⏰")
        st.warning(f"⏰ **Saatlik Hatırlatıcı ({row['Saat']}):** {row['Görev / Ders']} çalışmanız gerekiyor!")
...
with tab_tyt:
    ...
    if not tyt_df.empty:
        ...
        fig = px.line(toplam_tyt, x="Tarih", y="Net", text="Net", hover_data=["Yayın/Deneme Adı"], title="TYT Toplam Net Gelişimi", markers=True)
...
with tab_ayt:
    ...
    if not ayt_df.empty:
        ...
        fig_ayt = px.line(toplam_ayt, x="Tarih", y="Net", text="Net", hover_data=["Yayın/Deneme Adı"], title="AYT Toplam Net Gelişimi", markers=True)
...
with tab_brans:
    ...
    if not brans_df.empty:
        fig_brans = px.line(brans_df, x="Tarih", y="Net", color="Kayıt Türü", hover_data=["Yayın/Deneme Adı"], title=f"{secilen_brans} Net Gelişimi", markers=True)
...
with tab_konu:
    ...
    if tum_hatalar:
        ...
        # OTOMATİK UYARI SİSTEMİ (3 veya daha fazla hata yapılan konular)
        kritik_konular = hata_df[hata_df["Hata Sayısı"] >= 2]
        if not kritik_konular.empty:
            st.error("🚨 **AKILLI KOÇ UYARISI:** Aşağıdaki konularda üst üste hatalar yapıyorsunuz! Acil tekrar ve soru çözümü önerilir:")
            for _, row in kritik_konular.iterrows():
                st.warning(f"👉 **{row['Konu']}**: Toplam **{row['Hata Sayısı']}** kez yanlış yapıldı! (Programınıza bu konuyu ekleyin)")
...
with tab_program:
    ...
    duzenlenmis_program = st.data_editor(df_program, num_rows="fixed", use_container_width=True)
...
with tab_hatirlatici:
    ...
    st.write("Belirli saatlere özel ders veya konu hatırlatıcıları kurun (Örn: Saat 16:00 - Trigonometri):")
    
    col_s1, col_s2, col_s3 = st.columns([2, 4, 2])
    saat_input = col_s1.text_input("Saat (Örn: 16:00)", value="16:00")
    gorev_input = col_s2.text_input("Görev / Ders / Konu", placeholder="Örn: Trigonometri 50 Soru Çözümü")
    
    if col_s3.button("➕ Hatırlatıcı Ekle"):
        if saat_input and gorev_input:
            yeni_h = pd.DataFrame([{"Saat": saat_input, "Görev / Ders": gorev_input, "Durum": "Bekliyor"}])
